...
// Get full orphanage details by manager userId
router.get('/full/:userId', async (req, res) => {
    try {
        const { userId } = req.params;

        // Find orphanage where manager field matches userId
        const orphanage = await Orphanage.findOne({ manager: userId });

        if (!orphanage) {
            return res.status(404).json({ message: 'Orphanage not found for this manager' });
        }

        // Return the complete orphanage document
        res.status(200).json(orphanage);
    } catch (err) {
        console.error("Error fetching orphanage by userId:", err);
...
router.get('/manager/:userId', async (req, res) => {
    try {
        ...
        // Return the embedded orphans
        res.json(orphanage.orphans);
    } catch (err) {
        console.error("Error fetching orphans for manager:", err);
        res.status(500).json({ message: 'Server error' });
    }
});
